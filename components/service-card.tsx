...
export function ServiceCard({service}: { service: ServiceItem }) {
    return (
        <article
            className="surface-card card-stack group min-h-[17rem] border-t-4 border-t-[var(--brand-blue)] p-5 md:p-6">
                ...
                <ul className="mt-auto space-y-2 pt-2 text-sm font-medium leading-6 text-[var(--ink)]">
                    {service.points.map((point) => (
                        <li key={point} className="flex items-start gap-2.5">
                            <svg className="mt-0.5 h-4 w-4 shrink-0 text-[var(--brand-blue)]" fill="none"
